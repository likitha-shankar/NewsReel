...
**Three news source types + an enrichment pass.** RSS (Google News, any topic string), Hacker News Algolia API, and BeautifulSoup scrapers (Verge, BBC). But headlines alone starve the writer — a diagnostic showed RSS `summary` fields are just the title repeated, HN gives engagement counts, scrapers give nothing. No model can write 2,000 words from 15 headlines, so *switching models never fixed the length ceiling — starved input did.* The fix is an **enrichment stage** (`enrich_sources`): for the top articles per topic it fetches the body and TLDRs it with a cheap model (gpt-4o-mini), replacing the headline-only summary. Google News links are JS-obfuscated redirects and can't be read, so they stay headline-level; HN/scraper/listener links are direct publisher URLs and get enriched. This is the "summarize-first" context strategy: real substance for the writer, context window kept bounded (TLDR, not raw article), cost trivial (~$0.0001/article). Full-article RAG is the next step only if episodes need paragraph-level depth.

**Listener knowledge level scales the *data*, not just the tone.** Depth started as a pure style directive ("use jargon" for expert) layered over identical TLDRs — which meant "expert" episodes had the same shallow facts as "basic" ones, just described more technically. Fixed: `enrich_sources` now takes `depth` and scales both dimensions — expert reads 2 more articles per topic and TLDRs them to 6–8 sentences emphasizing technical detail and mechanisms, vs. 3–4 plain-language sentences for basic/balanced. Verified: the same article TLDR'd at 524 chars for basic vs. 935 chars for expert, with the expert version retaining specifics (context-window behavior, a concrete decision heuristic) the basic version correctly drops. Expert listeners get more substance to be technical *about*, not just permission to sound technical.
